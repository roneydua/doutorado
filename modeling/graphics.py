...
def plot_all_states(s,name=''):
    ...
    if name != '':
        plt.savefig(imageFolder+"all_states_"+name+'.pdf', format="pdf")
        plt.close(1)
    return True
...
def plot_deformations(s,name=''):
    ...
    for col in range(4):
        ax[0][col].plot(s.t[:-1],
                        s.deformation[col, :].T ,
                        label=s.accel_instance.leg[col])
        ax[0][col].set_yticks(
            [s.deformation[col, :].min(), s.deformation[col, :].max()])
        ax[0][col].legend()
        ax[1][col].plot(s.t[:-1],
                        s.deformation[col + 4, :].T ,
                        label=s.accel_instance.leg[col + 4])
        ax[1][col].set_yticks(
            [s.deformation[col+4, :].min(), s.deformation[col+4, :].max()])
        ax[1][col].legend()
        ax[2][col].plot(s.t[:-1],
                        s.deformation[col + 8, :].T ,
                        label=s.accel_instance.leg[col + 8])
        ax[2][col].set_yticks(
    ...
    if name != '':
        plt.savefig(imageFolder + "deformations_" +name + '.pdf', format="pdf")
        plt.close(2)
    return True
...
def plot_euler(s,name=''):
    ...
    if name != '':
        plt.savefig(imageFolder+'euler_'+name+'.pdf', format="pdf")
        plt.close(3)
    return True
...
def plot_accelerations(s,name=''):
    ...
    plt.show()
    if name != '':
        plt.savefig(imageFolder+'accelerations_'+name+'.pdf', format="pdf")
        plt.close(4)
    return True
...
def plot_accelerations_error(s,name=''):
    ...
    if name != '':
        plt.savefig(imageFolder+'accelerations_errors_'+name+'.pdf', format="pdf")
        plt.close(5)
    return True
