...
def plot_one_graphic(date: str, fbg_keys=[]):
    ...
    for i in fbg_keys:
        if i not in [
            ...
            ax.plot(
                ff[i + "/wavelength_m"][:] * 1e9, ff[i + "/reflectivity"][:, -1], label=i
            )
            ax.set_xlim(
    ...
    f.close()
    # plt.show()
    plt.savefig("fbg_production/" + date + append_name+".png", format="png",transparent=False)
    plt.close(fig=1)


# plot_one_graphic("20231130",fbg_keys=["fbg4","fbg9"])
plot_one_graphic('20240207')
...
def plot_graphics_with_pairs_acc_3():
    ...
    def  plot_fbg(ax:plt.axes,fbg_number:str):
        ax.plot(
            ff[fbg_number+"/wavelength_m"][:]*1e6, ff[fbg_number+"/reflectivity"][:, -1], label=fbg_number
        )
        ax.legend()
    f = h5py.File('./production_files.hdf5','r')
...
def plot_graphics_with_pairs_acc_5():
    ...
    fig.supxlabel(r"$\lambda [\unit{\nm}]$")
    fig.supylabel(r"Refletividade $[\unit{\percent}]$")
    plot_fbg(ax[0], fbg_number="fbg1", ff=ff)
    plot_fbg(ax[0], fbg_number="fbg16", ff=ff)
    plot_fbg(ax[1], fbg_number="fbg2", ff=ff)
    plot_fbg(ax[1], fbg_number="fbg17", ff=ff)
